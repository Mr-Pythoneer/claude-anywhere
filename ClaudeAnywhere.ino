/*
  ...
  Plug into any computer:
    ...
    • Custom API relay endpoint (for regions where api.anthropic.com
      is blocked, e.g. China) — set during WiFi setup portal

  LED    orange = connecting   blue = ready   rainbow = thinking
         green  = done         red  = error
  Button tap    → clear conversation
         hold 3s → reset WiFi (captive portal on next boot, also
                    lets you change/clear the custom API relay)
  ─────────────────────────────────────────────────────────────────
*/
...
const char* DEFAULT_CLAUDE_URL = "https://api.anthropic.com/v1/messages";
String claudeURL;   // loaded from flash in setup(); editable via WiFi setup portal
                     // (lets the device reach Claude through a relay in regions
                     // where api.anthropic.com is blocked, e.g. China)
...
void connectWiFi() {
  ...
  // Custom field: lets the device reach Claude through a relay where
  // api.anthropic.com is blocked (e.g. China). Blank = use default directly.
  WiFiManagerParameter custom_endpoint(
    "endpoint", "Claude API endpoint (blank = default)",
    claudeURL == DEFAULT_CLAUDE_URL ? "" : claudeURL.c_str(), 120);
  wm.addParameter(&custom_endpoint);
  ...
  // Pick up endpoint change only if the portal was actually shown & submitted
  // (when autoConnect reuses saved WiFi creds, getValue() just echoes the default)
  String entered = String(custom_endpoint.getValue());
  entered.trim();
  String newURL = entered.length() ? entered : String(DEFAULT_CLAUDE_URL);
  if (newURL != claudeURL) {
    claudeURL = newURL;
    prefs.putString("endpoint", claudeURL);
    Serial.print(F("[Config] API endpoint set to: "));
    Serial.println(claudeURL);
  }
...
void printBanner(const String& ip) {
  Serial.println();
  Serial.println(F("  ╔════════════════════════════════════════════╗"));
  Serial.println(F("  ║    CLAUDE ANYWHERE  v5.1                   ║"));
  Serial.println(F("  ║    AtomS3U · claude-sonnet-4-6             ║"));
  if (claudeURL != DEFAULT_CLAUDE_URL) {
    Serial.println(F("  ╠════════════════════════════════════════════╣"));
    Serial.println(F("  ║  Using custom API relay:                   ║"));
    Serial.print  (F("  ║    "));
    Serial.print(claudeURL);
    for (int i = claudeURL.length(); i < 40; i++) Serial.print(' ');
    Serial.println('║');
  }
  Serial.println(F("  ╠════════════════════════════════════════════╣"));
...
void setup() {
  ...
  prefs.begin("claudeanywhere", false);
  claudeURL = prefs.getString("endpoint", DEFAULT_CLAUDE_URL);
